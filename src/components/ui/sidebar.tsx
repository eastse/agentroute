...
const SIDEBAR_DRAG_THRESHOLD = 3;
...
function getSidebarWidthCookie(fallbackWidth: number) {
    ...
    const cookieValue = document.cookie
        .split("; ")
        .find((cookie) => cookie.startsWith(`${SIDEBAR_WIDTH_COOKIE_NAME}=`))
        ?.split("=")[1];
...
function SidebarProvider({
    ...
    const [sidebarWidth, _setSidebarWidth] = React.useState(() =>
        clampSidebarWidth(getSidebarWidthCookie(defaultWidth), minWidth, maxWidth)
    );
    ...
    const setSidebarWidth = React.useCallback(
        (value: number | ((width: number) => number)) => {
            _setSidebarWidth((currentWidth) => {
                const width = typeof value === "function" ? value(currentWidth) : value;
                const nextWidth = clampSidebarWidth(width, minWidth, maxWidth);
    ...
    React.useEffect(() => {
        _setSidebarWidth((currentWidth) => clampSidebarWidth(currentWidth, minWidth, maxWidth));
    }, [minWidth, maxWidth]);
...
function SidebarRail({
    ...
    const { state, setOpen, toggleSidebar, sidebarWidth, setSidebarWidth, setIsResizing } =
        useSidebar();
    ...
    const finishDrag = React.useCallback(
        (event: React.PointerEvent<HTMLButtonElement>) => {
            ...
            wasDraggingRef.current = dragState.didDrag;
    ...
    return (
        <button
            ...
            onPointerDown={(event) => {
                ...
                const side = event.currentTarget.closest("[data-side=right]") ? "right" : "left";
            ...
            onPointerMove={(event) => {
                ...
                if (Math.abs(deltaX) > SIDEBAR_DRAG_THRESHOLD) {
...
function SidebarSeparator({ className, ...props }: React.ComponentProps<typeof Separator>) {
    return (
        <Separator
            ...
            className={cn(
                "mx-2 max-w-[calc(100%-1rem)] bg-sidebar-border data-[orientation=horizontal]:w-[calc(100%-1rem)]",
                className
            )}
            {...props}
        />
